...
export default function App() {
  const { debug, gravity } = useControls({ 
    debug: false,
    gravity: { value: -40, min: -100, max: 0, step: 1 },
...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const cardColor = useMemo(() => new THREE.Color(hovered ? '#ff6b6b' : '#4ecdc4'), [hovered])
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial color="#feca57" depthTest={false} resolution={[width, height]} useMap map={texture} repeat={[-3, 1]} lineWidth={2} transparent opacity={0.75} />
      </mesh>
    </>
  )
}
